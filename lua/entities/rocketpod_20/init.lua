...
local ENERGY = 2000 //energy used to create rocket during reload phase
local CLIP = 20 //total capacity of rockets
local RELOAD = 2 //seconds it takes to make each bomb
local ROCKET_LIFE = 10 //seconds the rockets last in flight --was 14

function ENT:Initialize()

	self:SetModel( "models/Slyfo/missile_pod_10.mdl" ) 
...
function ENT:SpawnFunction( ply, tr )
	...
	local SpawnPos = tr.HitPos + tr.HitNormal * 52
	local ent = ents.Create( "rocketpod_20" )
...
function ENT:Use( ply )
	...
	ply:PrintMessage(HUD_PRINTCONSOLE,"Time delay between firing each rocket = 1/10 second")
...
function ENT:Launch()
	...
	if ( !Rocket:IsValid() ) then 
		...
		local y = math.Round(self.Magazine%2/10+1.4)*10-15
		local x = math.Round((self.Magazine%10+1)/2)*8-24
		Rocket:SetPos( self:GetPos() + self:GetUp()*y + self:GetRight()*x + Forward*66 + ( Vector(math.abs(Vel.x),math.abs(Vel.y),math.abs(Vel.z))*Forward ))
